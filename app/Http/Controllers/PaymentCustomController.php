...
class PaymentCustomController extends Controller
{
    public function getPayment(Request $request)
    {
        ...
        $data = DB::table('payments')
            ->select('patients.name', 'payments.desc', 'payments.type', 'payments.status', 'payments.id')
    ...
    public function getPaymentDetails(Request $request)
    {
        $data = DB::table('payments')
            ->select(
                'medicine_prescription.medicine_id',
                'medicine_prescription.prescription_id',
                'medicine_prescription.frequency',
                'medicine_prescription.quantity',
                'patients.name',
                'patients.address_1',
                'patients.address_2',
                'patients.state',
                'patients.postcode',
                'patients.phone',
                'medicines.name AS medsName',
                'medicines.price_per_unit AS medsPrice',
